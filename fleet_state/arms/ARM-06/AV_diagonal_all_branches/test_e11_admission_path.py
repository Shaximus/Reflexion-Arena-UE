...
Exit codes:  0 = an admission path exists AND discriminates correctly
             ...
             2 = cannot evaluate: NO admission path exists, or the harness is
                 not wired to a validator. NEVER reported as a pass.
             3 = an admission path EXISTS but this control cannot exercise it
                 (wrong surface). Distinct from 2 on purpose: "nothing to
                 check" and "something to check that I could not reach" are
                 opposite states, and collapsing them is DEFECT-3 one level up.
...
# Surface tokens. The first two are the effect's ENGLISH NAME; the rest are
# STRUCTURAL — the shape an authority gate takes regardless of wording.
#
# Measured 2026-08-05 against ARM-08's implementation: of 20 hits on the name
# alone, only 4 were code identifiers — 12 were string literals and 4 comments.
# The detector fired, but 80% of its evidence was prose. A functionally identical
# gate that never spells "adjust_counter" would have been INVISIBLE, and this
# control would have gone back to reporting a false absence. Detecting THIS
# implementation is not the same as detecting AN implementation.
E11_TOKENS = ("adjust_counter", "AdjustCounter",
              "AdmitCounterWrite", "EAdmission", "CounterAuthority",
              "bA2Writable", "A2Writable", "IsKnownCounter")
...
def main():
    ...
    if ue_hits:
        ...
        print("\n  exit 3 — the path exists and was NOT exercised. NOT a pass, NOT an")
        print("  absence finding, and NOT the same as exit 2 which means no path exists.")
        sys.exit(3)
