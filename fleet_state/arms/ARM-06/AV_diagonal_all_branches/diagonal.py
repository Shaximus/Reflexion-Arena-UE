...
def mut_correct_gate(root):
    """Give the mirror a CORRECT E11 gate: refuses the exploit, admits canon.

    The branch that matters most. D4 proved the control can condemn a broken
    gate; this asks whether it can RECOGNISE a correct one. A verifier that
    cannot tell a good implementation from a bad one is useless the day the
    real gate lands.
    """
    p = root / "mirror/tools/oracle/sim_mirror_rules.py"
    s = p.read_text()
    s = s.replace('LEGAL_EFFECTS = ["destabilize_anchor"]',
                  ...
                  '    A2_WRITABLE = {"boss_release_delay"}', 1)
    old = '        if str(spec.get("effect", "")) not in SkillSystem.LEGAL_EFFECTS:'
    new = ('        if str(spec.get("effect", "")) == "adjust_counter":\n'
           '            cid = spec.get("counter_id")\n'
           '            if cid not in SkillSystem.A2_WRITABLE:\n'
           '                return {"ok": False, "detail": "counter %r is not A2-writable" % cid}\n'
           '            return {"ok": True, "detail": ""}\n'
           '        if str(spec.get("effect", "")) not in SkillSystem.LEGAL_EFFECTS:')
    assert s.count(old) == 1
    p.write_text(s.replace(old, new, 1))


def mut_ue_present(root):
    """Put the gate's STRUCTURAL tokens in the UE tree, mirror unchanged.

    Should drive PRESENT_BUT_UNEXERCISED — path visible on one surface, not
    exercisable on the one this control probes.
    """
    d = root / "repo/Source/ReflexionArena/Sim"
    d.mkdir(parents=True, exist_ok=True)
    (d / "RxCounterAuthority.h").write_text(
        "// scratch fixture\nenum class EAdmission { Admit };\n"
        "bool IsKnownCounter(const char*);\n"
        "EAdmission AdmitCounterWrite(const char*);\n")


MUTS += [
    ("D5_correct_gate", mut_correct_gate, "test_e11_admission_path.py",
     "mirror implements a CORRECT allow-list: exploit refused, canon admitted"),
    ("D6_ue_present_only", mut_ue_present, "test_e11_admission_path.py",
     "gate tokens present in the UE tree, mirror unchanged"),
]
