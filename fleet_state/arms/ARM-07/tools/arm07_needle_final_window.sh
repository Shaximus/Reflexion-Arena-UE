...
cat > "$REC" <<EOF
{
  ...
  "purpose": "Foundry probe_beyond_native.py run verbatim on 5b (word-salad needle variant failed on instrument grounds 3x)",
  ...
  "expected_artifacts": "10_5b_needle_foundry_probe.json, 10_needlefinal_*",
...
python3 "$W/fleet_state/arms/ARM-07/tools/arm07_needle_final.py" "$EV"
E=$?
echo "DRIVER_EXIT=$E"
